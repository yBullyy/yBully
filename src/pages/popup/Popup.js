  ...
  const onClickHandler = async (isSignup) => {
    ...
    if (user?.uid) {
      ...
      if (isSignup) {
        ...
        let isSuccess = await saveUserToFirestore({ email: user.email, username, uid: user.uid, isVerified: user.emailVerified });
      ...
      if (user.emailVerified) {
        ...
        alert('Please verify your email using the link sent to your email and then login again');
      }
    }
  }
